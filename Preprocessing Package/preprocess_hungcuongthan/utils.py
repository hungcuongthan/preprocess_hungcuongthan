...
def _get_avg_wordlength(x):
    return round(_get_charcounts(x) / _get_wordcounts(x),2)
...
def _get_contraction_expansion(x):
    contractions = { 
        "ain't": "am not",
        "aren't": "are not",
        "can't": "cannot",
...
def _remove_rt(x): #remove retweet
    return re.sub(r'\btr\b','',x).strip()

def _remove_special_chars(x): #remove special chars and multiple spaces
    x = re.sub(r'[^\w ]+', '',x)
    x = ' '.join(x.split())
...
def _remove_common_words(x, n = 20):
    ...
    f_top_n = freq_comm[:n]
    x = ' '.join([t for t in x.split() if t not in f_top_n])
    return x
